...
public class StatisticsValidationsTestBed {

	protected static Injector injector = null;

	protected final DatabaseStatisticsLogger logger;

	protected static String str10;
	protected static String str51;
	protected static String str256;
	
	public StatisticsValidationsTestBed(){
		...
		str10 = getRandomStringPlusOne(11);
		str51 = getRandomStringPlusOne(51);
		str256 = getRandomStringPlusOne(256);
	...
	public void testDB(){
		...
		entry.setDocumentType( str51 );
		entry.setGeneratedBy( "Branch: JUnit Test" );
		entry.setExportTo( str256 );
		logger.store( entry );
		
		StatsEntry logged = (StatsEntry) logger.fetchLatest().get( 0 );
		
		Assert.assertThat( "Document Type: ", logged.getDocumentType(), CoreMatchers.is( str51.substring(0,49) ));
		Assert.assertThat( "Generator: ", logged.getGeneratedBy(), CoreMatchers.is("Branch: JUnit Test"));
	...
	public void testComplete(){
		...
		StatsEntry entry = new StatisticsManager( MockObjects.complete() ).prepare(); 
		
		entry.setDocumentType( "ECV" );
		entry.setFileFormat( str51 );
		entry.setGeneratedBy( "Branch: JUnit Test Complete" );
		entry.setExportTo( str51 );
		logger.store( entry );
		
		StatsEntry logged = (StatsEntry) logger.fetchLatest().get( 0 );
		
		Assert.assertThat( "Document Type: ", logged.getDocumentType(), CoreMatchers.is( "ECV" ));
		Assert.assertThat( "File Format: ", logged.getFileFormat(), CoreMatchers.is( str51.substring(0,49) ));
		Assert.assertThat( "Generator: ", logged.getGeneratedBy(), CoreMatchers.is("Branch: JUnit Test Complete"));
		Assert.assertThat( "Export Destination: ", logged.getExportTo(), CoreMatchers.is(str51.substring(0,49)));
	...
	public void testEntryAndNationalityDB(){
		...
		for ( StatsNationality loggedNat : loggedNationalities ){
			Assert.assertThat( "Nationality name", loggedNat.getNationality(), CoreMatchers.is( names[i].substring(0, 254) ) );
			i++;
	...
	public void testEntryAndAchievementsDB(){
		...
		for ( StatsAchievement loggedItem : loggedList ){
			Assert.assertThat( "Achievement category", loggedItem.getCategory(), CoreMatchers.is( names[i].substring(0, 254) ) );
			i++;
	...
	public void testEntryAndMotherTongueDB(){
		...
		for ( StatsMotherLanguage loggedItem : loggedList ){
			Assert.assertThat( "Mother Tongue name", loggedItem.getLanguage(), CoreMatchers.is( names[i].substring(0, 254) ) );
			i++;
	...
	public void testEntryAndWorkDB(){
		...
		String[] positions = {str256, str256};
		...
		item2.setPeriodFrom( new DateTime(2005,1,1,0,0, DateTimeZone.UTC) );
		...
		for ( StatsWorkExperience loggedItem : loggedList ){
			Assert.assertThat( "Work name", loggedItem.getPosition(), CoreMatchers.is( positions[i].substring(0, 254) ) );
			i++;
	...
	public void testEntryAndEduDB(){
		...
		entry.setGeneratedBy( "Branch: JUnit Test for Education Experience" );
		
		String[] titles = {str256, str256, str256};
		StatsEducation item1 = new StatsEducation();
		item1.setQualification( titles[0] );
		item1.setQualificationLevel( str256 );
		item1.setEducationalField( str256 );
		item1.setPeriodFrom( new DateTime(2000,1,1,0,0, DateTimeZone.UTC) );
		item1.setPeriodTo( new DateTime(2005,1,1,0,0, DateTimeZone.UTC) );
		
		StatsEducation item2 = new StatsEducation();
		item2.setQualification( titles[1] );
		item2.setQualificationLevel( str256 );
		item2.setOrganisationCountry( str256 );
		...
		item3.setQualificationLevel( str256 );
		...
		for ( StatsEducation loggedItem : loggedList ){
			Assert.assertThat( "Edu name", loggedItem.getQualification(), CoreMatchers.is( titles[i].substring(0, 254) ) );
			...
			switch( i ){
				case 0: {
					Assert.assertThat( "Edu 0 Field", loggedItem.getEducationalField(), CoreMatchers.is( str256.substring(0, 254) ) );
					break;
				}
				case 1:{
					Assert.assertThat( "Edu 1 Organisation Country", loggedItem.getOrganisationCountry(), CoreMatchers.is( str256.substring(0, 254) ) );
					break;
				}
			}
			i++;
	...
	public void testEntryAndForeignAndCertificateDB(){
		...
		entry.setGeneratedBy( "Branch: JUnit Test for Foreign Languages" );
		
		String[] titles = {str256, str256, str256};
		String[] certs = {str256, str256, str256};
		StatsForeignLanguages item1 = new StatsForeignLanguages();
		item1.setLanguageType(  titles[0] );
		item1.setListeningLevel( str10 );
		...
		for ( StatsForeignLanguages loggedItem : loggedList ){
			Assert.assertThat( "Foreign Language name", loggedItem.getLanguageType(), CoreMatchers.is( titles[i].substring(0, 254) ) );
			
			switch( i ){
				case 0: {
					Assert.assertThat( "Foreign Language 0 Listening Level", loggedItem.getListeningLevel(), CoreMatchers.is( str10.substring(0, 9) ) );
					break;
				}
				case 1:{
					...
					Assert.assertThat( "1st Linguistic Certificates Level", loggedCerts.get( 0 ).getCefrLevel(), CoreMatchers.is( str256.substring(0, 254) ) );
	...
	private String getRandomStringPlusOne(int size){
