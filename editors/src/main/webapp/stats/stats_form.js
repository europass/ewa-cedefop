var stats = {
    ...
    init: function (mode) {
        ...
        if (mode != undefined && mode != null && mode == "triggerLoad")
        ...
        for (var section in stats.sections) {
            ...
            for (var param in properties[section]) {
                ...
                if (!isNullOrUndef(param) && !isNullOrUndef(properties[section][param].descr)) {
                    ...
                    $selectEl.append(
                            $("<option>", {value: param, "text": properties[section][param].descr})
                            );
                }
            }
        }
    ...
    show: function ($messageContainer, message) {
        ...
        } //else show on a default container?
    ...
    update: function () {
        ...
        stats.dataFmt = $("fieldset#download-options").find("[data-attr='dataFmt']").val();
        stats.dataType = $("fieldset#query_params").find("[data-attr='dataType']").val();
...
//						if ( !isNullOrUndef(paramVal) && paramVal.length>0 )
        ...
        rawFile.onreadystatechange = function ()
        {
            if (rawFile.readyState === 4)
            {
                if (rawFile.status === 200 || rawFile.status == 0)
                {
                    ...
                    var props = contents.split("\n");
                    if (props.length == 1) {
                        var attrs = props[0].split("=");
                        if (attrs.length == 2) {
                            if (attrs[0] == "stats.api.request.url")
                                that.statsAPIurl = attrs[1];
                        }
                    }
                }
            }
    ...
    /**
     * Prepares the service url and prefixes
     */

    prepareLocations: function () {

        //var url = "http://localhost:8080/stats-api/";
        this.readTextFile();
        var url = this.statsAPIurl;

        var dataFmt = "to/" + stats.dataFmt + "/";
        var dataType = stats.dataType + ";";

        url += dataFmt + dataType;
        this.statsAPIurlPrefix = url;
    },
    ...
    getURL: function () {
        ...
        this.prepareLocations();
        var url = this.statsAPIurlPrefix;
        ...
        for (var section in stats.sections) {
            for (key in stats.params[section]) {
                ...
                // append = only uf there are values of the parameter
                url += key + "=";
     ...
     */
    getStatsFromAPI: function (resultsDisplayEl, presetUrl) {
        //build the URL depending on input values

        var url = stats.getURL();
        if (presetUrl !== undefined && presetUrl !== null)
            url = presetUrl;

        console.log("Query (results):'" + url + "'");
//			var url = "https://europassdev1.instore.gr/stats/to/csv/generated;date;document-type=ECV;gender;language";
//			stats.dataFmt = "csv";
        ...
        $.ajax({
            ...
            error: function (data) {
             ...
             always: function() {
             stats.canSend = true;
             $("body").trigger("statistics:preview:loading:hide");
             }*/
        });
        //return  result;
...
$().ready(function () {
            ...
            if (!isNullOrUndef(el.val()) && el.val() !== "") {
                ...
                if (!isNullOrUndef(properties[params]) && !isNullOrUndef(properties[params][paramName])) {
                    ...
                    if (typeof paramType === "string") {
                        ...
                        $valContainer.siblings(".x-axis-reset").show();
    ...
    $(".x-axis-addParameter").on("click", function (e) {
        ...
        if (paramName == undefined || paramName == null || paramName == "") {
            $("#query_params span.error").html("You should at least select a parameter before adding a new one...");
            return;
        } else
            $("#query_params span.error").html("");
...
//		$newParam.find(".x-axis-reset").hide(); //hide the param reset button

        // hide add button in case it is the last parameter permitted
        var parametersNumber = $(".x-axis .parameter").size();
        if (parametersNumber + 1 == maxParameters) {

            $newParam.find(".x-axis-addParameter").hide();
            $newParam.find(".x-axis-removeParameter").show();
        } else {
        ...
        $paramEl.find(".x-axis-addParameter").hide();
        $paramEl.find(".x-axis-removeParameter").show();
    ...
    $(".x-axis-removeParameter").on("click", function (e) {
        var $el = $(e.target);
        ...
        var currentNumber = $(".x-axis .parameter").size();

        $(".x-axis .parameter").each(function (idx) {

            if (currentNumber == 1) {
                $(this).find(".x-axis-removeParameter").hide();
            }

            if (idx + 1 == currentNumber) {
                $(this).find(".x-axis-addParameter").show();
//				$(this).find(".x-axis-removeParameter").show();
            } else if (idx + 1 < currentNumber) {
                $(this).find(".x-axis-addParameter").hide();
            }
    ...
    $(".x-axis-reset").on("click", function (e) {
        reset(e);
    });
...
function csvToTable(csv) {
    ...
    table.find("tbody tr:first").addClass("r_header");
    table.find("tbody tr:odd").addClass("r_odd");
    table.find("tbody tr:even:not(:first)").addClass("r_even");
...
function storeBookmarkedQuery(name, value) {
    var cookie = [name, '=', JSON.stringify(value), '; domain=.', window.location.host.toString(), '; path=/;'].join('');
    document.cookie = cookie;
}

function getBookmarkedQueries(name) {
    var result = document.cookie.match(new RegExp(name + '=([^;]+)'));
    result && (result = JSON.parse(result[1].replace(/\?/g, ";")));
    return result;
}

function deleteBookmarkedQueries(name) {
    document.cookie = [name, '=; expires=Thu, 01-Jan-1970 00:00:01 GMT; path=/; domain=.', window.location.host.toString()].join('');
}

function updateBookmarkedList(bookmarkedQueries) {

    if (bookmarkedQueries.length > 0) {

        var optionsHtml = "";

        for (var i = 0; i < bookmarkedQueries.length; i++) {

            var bookmark = bookmarkedQueries[i];
            var label = bookmark.split(stats.statsAPIurlPrefix)[1].replace(/;/g, " ");

            optionsHtml += '<option value="' + bookmark + '">' + label + '</option>';
        }

        $("select[data-attr='bookmarks']").html(optionsHtml);
        $("button#make_chart_preview_data_bookmark").removeAttr("disabled");
    } else {
        $("select[data-attr='bookmarks']").html("");
        $("button#make_chart_preview_data_bookmark").attr("disabled", "disabled");
