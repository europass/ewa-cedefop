...
<xsl:stylesheet 
    ...
    xmlns:europass_prev="http://europass.cedefop.europa.eu/Europass/V3.1"
    xmlns:europass="http://europass.cedefop.europa.eu/Europass/V3.2">
    
    <xsl:output method="xml" encoding="utf-8" indent="yes"/>
    <xsl:strip-space elements="*"/>
    
    <xsl:template name="addBundles">
        <xsl:param name="docType" />
        
        <xsl:choose>
            <xsl:when test="$docType='ECV_ESP'">
                <xsl:if test="node()|@*[name()='CoverLetter']">
                    <xsl:element name="Bundle">
                        <xsl:element name="DocumentType">ECL</xsl:element>
                    </xsl:element>
                </xsl:if>
            </xsl:when>
            
            <xsl:when test="$docType='ESP'">
                    <xsl:if test="node()|@*[name()='Identification']">
                        <xsl:element name="Bundle">
                            <xsl:element name="DocumentType">ECV</xsl:element>
                        </xsl:element>
                    </xsl:if>
                    <xsl:if test="node()|@*[name()='CoverLetter']">
                        <xsl:element name="Bundle">
                    ...
                    <xsl:if test="node()|@*[name()='AcquiredDuring']">
                        <xsl:element name="Bundle">
                            <xsl:element name="DocumentType">ELP</xsl:element>
                        </xsl:element>
                    </xsl:if>
            </xsl:when>
            
            <xsl:when test="$docType='ECV'">
                    <xsl:if test="node()|@*[name()='CoverLetter']">
                        <xsl:element name="Bundle">
                            <xsl:element name="DocumentType">ECL</xsl:element>
                        </xsl:element>
                    ...
                    <xsl:if test="node()|@*[name()='AttachmentList']">
                        <xsl:element name="Bundle">
                            <xsl:element name="DocumentType">ESP</xsl:element>
                        </xsl:element>
                    </xsl:if>
            </xsl:when>
            
            <xsl:when test="$docType='ELP'">
    ...
    <xsl:template match="node()|@*">
        ...
        <xsl:choose>
            ...
            <xsl:when test="current()[ name()='DocumentType']">
                <xsl:copy-of select="."/>
                    <xsl:call-template name="addBundles">
                        <xsl:with-param name="docType" select="current()" />
                    </xsl:call-template>
            </xsl:when>
    ...
    <!--<xsl:template match="PrintingPreferences/Document/Field[starts-with(@name, 'LearnerInfo.Identification.ContactInfo.Email')]" />-->
