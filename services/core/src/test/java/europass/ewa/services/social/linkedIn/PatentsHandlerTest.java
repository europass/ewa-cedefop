...
public class PatentsHandlerTest {

	private static Injector injector = null;
	private static PatentsHandler handler = null;
	
	public PatentsHandlerTest(){
		injector = Guice.createInjector(
				new LinkedInModule()
				);
		
		handler = injector.getInstance(PatentsHandler.class);
	}
	...
	public void test() throws InstanceClassMismatchException{
		...
		handler.transform(ExtraDataMockObjects.patentsHashMapObject(), to, "SocialAchievementType", "patents", new Locale("el"), new DateConfig());
		
		Assert.assertTrue(to.size() == 1);
		
		Achievement toItem = to.get(0);
		
		Assert.assertThat("Title Label", toItem.getTitle().getLabel(), CoreMatchers.is( "Ευρεσιτεχνίες" ));
		
		String expected = ExtraDataMockObjects.PATENTS_HTML_EXPECTED;
