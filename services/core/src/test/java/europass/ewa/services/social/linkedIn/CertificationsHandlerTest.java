...
public class CertificationsHandlerTest {

	private static Injector injector = null;
	private static CertificationsHandler handler = null;
	
	public CertificationsHandlerTest(){
		injector = Guice.createInjector(
				new LinkedInModule()
				);
		
		handler = injector.getInstance(CertificationsHandler.class);
	}
	...
	public void test() throws InstanceClassMismatchException{
		...
		to = (List<Achievement>)handler.transform(ExtraDataMockObjects.certificationsHashMapObject(), to, "SocialAchievementType", "certifications", new Locale("el"));
		
		Assert.assertTrue(to.size() == 1);
		
		Achievement toItem = to.get(0);
		
		Assert.assertThat("Title Label", toItem.getTitle().getLabel(), CoreMatchers.is( "Πιστοποιήσεις" ));
		
		String expected = ExtraDataMockObjects.CERTIFICATIONS_HTML_EXPECTED;
