...
public class PublicationHandlerTest {
	...
	private static PublicationsHandler handler = null;
	
	public PublicationHandlerTest(){
		injector = Guice.createInjector(
				new LinkedInModule()
				);
		
		handler = injector.getInstance(PublicationsHandler.class);
	}
	...
	public void test() throws InstanceClassMismatchException{
		...
		to = (List<Achievement>)handler.transform(ExtraDataMockObjects.publicationsHashMapObject(), to, "AchievementType", "publications", new Locale("el"), new DateConfig());
		
		Assert.assertTrue(to.size() == 1);
		
		Achievement toItem = to.get(0);
		
		Assert.assertThat("Title Label", toItem.getTitle().getLabel(), CoreMatchers.is( "Δημοσιεύσεις" ));
		
		String expected = ExtraDataMockObjects.PUBLICATIONS_HTML_EXPECTED;
