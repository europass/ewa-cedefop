...
public class ProjectsHandlerTest {
	...
	private static ProjectsHandler handler = null;
	...
	public ProjectsHandlerTest(){
		...
		handler = injector.getInstance(ProjectsHandler.class);
	}
	...
	public void test() throws InstanceClassMismatchException{
		...
		to = (List<Achievement>)handler.transform(ExtraDataMockObjects.projectsHashMapObject(), to, "AchievementType", "projects", new Locale("el"));
		
		Assert.assertTrue(to.size() == 1);
		
		Achievement toItem = to.get(0);
		
		Assert.assertThat("Title Label", toItem.getTitle().getLabel(), CoreMatchers.is( "Προγράμματα" ));
		
		String expected = ExtraDataMockObjects.PROJECTS_HTML_EXPECTED;
