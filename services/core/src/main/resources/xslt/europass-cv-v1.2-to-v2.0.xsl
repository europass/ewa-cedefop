...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
							  ...
							  xmlns:europass1="http://europass.cedefop.europa.eu/Europass/V1.2">
	...
	<xsl:template name="docinfo">
		...
		<xsl:element name="comment">Automatically generated Europass CV  converted from 1.2 to 2.0</xsl:element>
	</xsl:template>
	...
	<xsl:template name="identification">
		...
		<xsl:element name="contactinfo">
			<xsl:element name="address">
				<xsl:element name="addressLine"><xsl:value-of select="contactinfo/address/addressLine" /></xsl:element>
				<xsl:element name="municipality"><xsl:value-of select="contactinfo/address/municipality" /></xsl:element>
				<xsl:element name="postalCode"><xsl:value-of select="contactinfo/address/postalCode" /></xsl:element>
				<xsl:element name="country">
					<xsl:element name="code"><xsl:value-of select="contactinfo/address/country/code" /></xsl:element>
					<xsl:element name="label"><xsl:value-of select="contactinfo/address/country/label" /></xsl:element>
				</xsl:element>
			</xsl:element>
		...
		<xsl:element name="demographics">
			...
			<xsl:element name="nationality">
				<xsl:element name="code"><xsl:value-of select="demographics/nationality/code" /></xsl:element>
				<xsl:element name="label"><xsl:value-of select="demographics/nationality/label" /></xsl:element>
			</xsl:element>
		</xsl:element>
		...
		<xsl:element name="diploma">
			...
			<xsl:if test="date != 'null'">
				<xsl:element name="date">
					<xsl:if test="date/year != 'null'" >
						<xsl:element name="year"><xsl:value-of select="date/year"/></xsl:element>
					</xsl:if>
					<xsl:if test="date/month != 'null'" >
						<xsl:element name="month"><xsl:value-of select="month"/></xsl:element>
					</xsl:if>
					<xsl:if test="date/day != 'null'" >
						<xsl:element name="day"><xsl:value-of select="date/day"/></xsl:element>
					</xsl:if>
				</xsl:element>
			</xsl:if>
