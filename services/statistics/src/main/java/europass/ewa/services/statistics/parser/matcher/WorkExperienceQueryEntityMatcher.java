 ...
 * @author at
 */
public class WorkExperienceQueryEntityMatcher extends QueryEntityMatcher {

	protected List<NumberValueRange> workExperienceCubeRangesList;

	public WorkExperienceQueryEntityMatcher(QueryEntityMatcher next) {
		super(next);
	...
	protected void configure() {
		...
		super.setMatch(new QueryEntityMatch(EntityTablesProperties.CUBE_ENTRY_WORKEXP, parametersList));
	}

	@Override
	public void dispatch(QueryEntityMatch matcher, QueryProperties queryProperties) {

		if (matcher.getParameters().contains(ParameterNames.WORK_EXPERIENCE)
	...
	protected void populateQueryEntity(QueryEntityMatch matcher, QueryProperties queryProperties) {
		...
		QueryParameter workExpParamater = queryProperties.getByParameterName(ParameterNames.WORK_EXPERIENCE);
		ValueProperties valueProperties = workExpParamater.getValueProperties();
		...
		if (valueProperties.getValueType().equals(ValueTypes.NUMBER_RANGE)) {
			...
			if (listContainsAllFromSublist(intValueRangeList, workExperienceCubeRangesList)) {
				matcher.setEntityProperties(EntityTablesProperties.CUBE_ENTRY_WORKEXP);
			} else {
				matcher.setEntityProperties(EntityTablesProperties.CUBE_ENTRY);
			}
		} else {
	...
	private void configureValueRanges() {
		workExperienceCubeRangesList = CubeEntryWorkExp.getRangesListValues();
	}
